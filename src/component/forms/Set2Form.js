...
class Set2Form extends Component {
    constructor(props) {
        super(props);
        this.state = {
            moistset : null,
    ...
    onSubmit(){
        Firebase.database().ref('Setting2').update({
            Moist : this.state.moistset*1,
        });
        Firebase.database().ref('Setting2/TimeWater').update({
            hr : this.state.waterhr*1,
            min : this.state.watermin*1,
        });
        Firebase.database().ref('Setting2/Timepui').update({
